...
public class SelectTest {
    ...
    @Test
    void selectFakeTable() {

        String sql = "select * from (select 1 from person ) as fake";
        final String handler = logicDeleteHandler.processSql(sql);
        Assertions.assertEquals("select * from ( select 1 from person where person.deleted = 0 ) fake", handler);
    }
